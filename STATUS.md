...
Snapshot date:
- `2026-07-03` (post-Stage 35 hardening, operator export, README refresh)

Last validated test sweep:
- Windows/WIP sweep on 2026-07-03: added hardened container policy modes
  (`seccomp=strict`, `cgroup=isolated`), `pg_qx_stat_prune_history()`,
  `pg_stat_qx_operator_export`, README SVG design refresh, and stage-doc
  normalization. WSL Ubuntu 24.04 now responds again; `/dev/kvm`,
  `scripts/ci/kvm-probe.sh`, and a direct QEMU/KVM boot marker
  `QX-MICROVM-BOOT-OK` all passed.
...
| 24 container backend scaffold | yes | yes | yes | yes | OCI policy compilation from capability tags (`readonly_rootfs`, `seccomp_mode`, `cgroup_mode`, `oci_profile`), strict/runtime-default/unconfined seccomp modes, private/isolated/host cgroup modes, image allowlist enforcement, runner `docker run` honors compiled policy, and `qx_stage24_container_policy` regress; full rootless OCI and kernel-level cgroup namespace ownership remain intentional deferrals |
...
| 33 stats collector and runtime hardening | yes | yes | partial | partial | `qx_stat` shared-memory collector, `qhapaqxian.track_stats`, catalog scheduler helpers, runtime policy/supervisor, typed launch requests, KVM CI workflow, `qx_stage3_observability`, Stage 26–27 scheduler/recovery collector SRFs, durable history pruning/export, and richer OCI policy modes landed; full rootless OCI and VM process-control ownership remain open |
| 34 observability collector v2 | yes | yes | yes | partial | `pg_qx_stat_get_principal_stats`, `pg_qx_stat_get_runtime_class_stats`, collector-backed `pg_stat_qx_principals`/`pg_stat_qx_runtime_classes`, Stage 26–27 scheduler/recovery collector views, durable `pg_qx_stat_history` with `pg_qx_stat_snapshot()`/`pg_qx_stat_get_history()`/`pg_qx_stat_prune_history()`, `pg_stat_qx_slo_providers`, `pg_stat_qx_operator_dashboard`, `pg_stat_qx_operator_export`, extended `qx_stage3_observability`, and `docs/stage-34/README.md` landed; automatic snapshot scheduling and graphical operator UI remain open |
| 35 CI loopback path, backend IDs, catalog runtime writes | yes | yes | yes | partial | `qhapaqxian_output` loopback resume path, real `container_id`/`vm_id` trace evidence, meson microVM/container env wiring, `qx_catalog` runtime update/insert helpers, duplicate receipt-tail stripping, cgroup/seccomp-aware OCI profile compilation with runner `--cgroupns` wiring, shared-memory `pg_stat_qx_backend_supervisor` plus `pg_qx_backend_supervisor_list()`, and cross-session `qx_stage35_supervisor` regress; kernel-level cgroup ownership and external process termination remain future work |

Canonical next-gap summary:
- strongest remaining platform gap: materialize full OCI/rootless profiles and external VM/container process-control ownership beyond the shared supervisor registry;
- strongest observability gap: add automatic snapshot scheduling and a graphical/external operator UI on top of the SQL dashboard/export surfaces;
- strongest documentation gap: older stage docs still vary in depth, but `README.md`, `docs/README.md`, and current runtime/observability stage docs now define the cleanup baseline.
