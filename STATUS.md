...
| 32 catalog snapshot helper layer | yes | yes | partial | partial | recovery, planner, security, commands, runtime scheduler reads, runtime insert/ledger writes, and provider/principal/runtime-class stats SRFs now use `qx_catalog` helpers; remaining gaps are security DDL/write paths and dedicated helper-layer regression coverage |
